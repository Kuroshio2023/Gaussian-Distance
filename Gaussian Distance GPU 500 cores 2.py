...
def compute_distance_matrix_gpu(data, result_matrix):
    ...
    if i < n and j < n:
        ...
        def gaussian_dist(t, x1, y1, x2, y2, s=0.304):
            return cp.linalg.norm(
                (x1 * t + y1 * (1 - t)) * norm.pdf(t, 0.5, s) -
                (x2 * t + y2 * (1 - t)) * norm.pdf(t, 0.5, s)
            )

        result, _ = quad(gaussian_dist, -np.inf, np.inf, args=(x1, y1, x2, y2))
